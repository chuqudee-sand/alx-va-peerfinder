...
CREDS_FILE = 'C:/Users/niky4/peer_grouping_app/peer-finder-data-sheet-327f56f0522e.json'  # Put your JSON key here
SPREADSHEET_ID = '1oh89n1ESIZIt_BK28l9-NgWV_A2d0Q5mnRaIV5Ca8Hk'  # Extract from your Google Sheet URL

def get_sheet():
    creds = ServiceAccountCredentials.from_json_keyfile_name(CREDS_FILE, SCOPE)
    client = gspread.authorize(creds)
    sheet = client.open_by_key(SPREADSHEET_ID).sheet1  # Use first sheet or by name
    return sheet
...
def read_queue():
    records = read_data()
    if not records:
        columns = ['id', 'name', 'phone', 'email', 'submitted', 'timestamp', 'matched', 'group_size', 'group_id']
...
def save_queue(df):
    # Google Sheets API does not allow direct overwrite of all data easily
    # Instead, clear the sheet and rewrite all rows (including header)
    sheet = get_sheet()
    sheet.clear()
